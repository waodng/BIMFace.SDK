...
namespace BIMFace.SDK.CSharp.Common.Http
{
    ...
    public partial class HttpManager
    {
        ...
        /// <param name="url">请求目标URL</param>
        /// <returns>HTTP-POST的响应结果</returns>
        public async Task<HttpResult> PatchAsync(string url)
        {
            return await RequestStringAsync(url, null, HttpMethodValues.PATCH, null);
        }
        ...
        /// <param name="contentType"><see langword="Content-type" /> HTTP 标头的值。请使用 HttpContentType 类的常量来获取</param>
        /// <returns>HTTP-POST的响应结果</returns>
        public async Task<HttpResult> PatchAsync(string url, string data, string contentType = HttpContentType.APPLICATION_JSON)
        {
            return await RequestStringAsync(url, data, HttpMethodValues.PATCH, contentType);
        }
